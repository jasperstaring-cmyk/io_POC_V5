...
// ─── EnterpriseFlow — accordeon aanpak ───────────────────────────────────────
// Alle secties op één pagina. Secties unlockten sequentieel.
// Props:
//   onComplete   — callback na bevestiging
//   onBack       — callback terug
//   profileData  — { firstName, lastName, email, jobRole } — al bekend via profile-first
//   segmentData  — { segment, orgType } — optioneel, meegegeven vanuit BusinessFlow/upstream
...
export default function EnterpriseFlow({ onComplete, onBack, profileData, segmentData }) {
  ...
  const [dnbPhase, setDnbPhase]       = useState("lookup")   // "lookup" | "confirm" | "manual"
  ...
  // ── Editie ────────────────────────────────────────────────────────────────
  const [edition, setEdition] = useState(null)   // "nl" | "international"

  // ── Toegangsmethode + telefoon ────────────────────────────────────────────
  const [accessMethod, setAccessMethod] = useState(null)  // "domain" | "sso" | "unknown"
  ...
  function resetFrom(id) {
    const idx = SECTIONS.indexOf(id)
    setDoneSection(prev => prev.filter(s => !SECTIONS.slice(idx).includes(s)))
  }
  ...
  // ── D&B typeahead ─────────────────────────────────────────────────────────
  useEffect(() => {
    if (searchMode !== "name" || searchQuery.length < 3) { setSearchResults([]); setShowDropdown(false); setNoResults(false); return }
    setIsSearching(true); setNoResults(false)
    clearTimeout(searchTimeout.current)
    searchTimeout.current = setTimeout(() => {
      const results = searchByName(searchQuery, company.country)
      setSearchResults(results); setShowDropdown(true); setIsSearching(false); setNoResults(results.length === 0)
    }, 300)
  ...
  function confirmSegment()          { if (!segment || !orgType) return; markDone("segment_type"); openNext("segment_type") }
  function confirmEdition(val)       { setEdition(val); markDone("edition"); openNext("edition") }
      ...
      <div style={{ minHeight:"100vh", background:C.gray50 }}>
        ...
        <div style={{ display:"flex", alignItems:"center", justifyContent:"center", minHeight:"calc(100vh - 56px)", padding:"2rem" }}>
          <div style={{ background:C.white, borderRadius:12, padding:"2.5rem 3rem", maxWidth:580, width:"100%", boxShadow:"0 4px 24px rgba(12,24,46,0.08)" }}>
            ...
            <button className="btn-red btn-full" onClick={onComplete}>{t("ef_confirm_cta")}</button>
          </div>
        </div>
      </div>
    )
    ...
    <div className="reg-layout">
      ...
      <div className="reg-container">
        <div className="reg-main">
          ...
          {/* Profielchip */}
          {profileData && (
            <div style={{ background:C.gray50, border:`1px solid ${C.gray200}`, borderRadius:8, padding:"0.75rem 1rem", marginBottom:"1.75rem", display:"flex", alignItems:"center", gap:"0.75rem" }}>
              <div style={{ width:32, height:32, borderRadius:"50%", background:C.navy, display:"flex", alignItems:"center", justifyContent:"center", flexShrink:0 }}>
                <span style={{ fontFamily:"var(--font-sans)", fontSize:"0.75rem", fontWeight:700, color:C.white }}>
                  {(profileData.firstName?.[0] || "").toUpperCase()}{(profileData.lastName?.[0] || "").toUpperCase()}
                </span>
              </div>
              <div>
                <div style={{ fontFamily:"var(--font-sans)", fontSize:"0.875rem", fontWeight:600, color:C.navy }}>
                  {profileData.firstName} {profileData.lastName}
                  {profileData.jobRole && <span style={{ fontWeight:400, color:C.gray500 }}> · {profileData.jobRole}</span>}
                </div>
                <div style={{ fontFamily:"var(--font-sans)", fontSize:"0.8rem", color:C.gray500 }}>{profileData.email}</div>
              </div>
            </div>
          )}
          ...
          <Accordion
            ...
            onOpen={() => !isDone("company") || setOpenSection("company")}
            onEdit={() => { setOpenSection("company"); resetFrom("company") }}
          >
            {/* Lookup fase */}
            {dnbPhase === "lookup" && <>
          ...
          {/* ── SECTIE 2: Segment + type (alleen als niet upstream al bekend) ── */}
          {!hasSegmentData && (
            <Accordion
              ...
              onOpen={() => isUnlocked("segment_type") && setOpenSection("segment_type")}
              onEdit={() => { setOpenSection("segment_type"); resetFrom("segment_type") }}
            >
              ...
              <SegmentTypeSelector segments={SEGMENTS} selectedSegment={segment} selectedType={orgType}
                onSelect={(seg, tp) => { setSegment(seg); setOrgType(tp) }} t={t} tSeg={tSeg} tType={tType} />
              <div style={{ marginTop:"1.25rem" }}>
                <button className="btn-green btn-full" disabled={!segment || !orgType} onClick={confirmSegment}>{t("bf_further")}</button>
              </div>
            </Accordion>
          )}
          ...
          <Accordion
            ...
            onOpen={() => isUnlocked("edition") && setOpenSection("edition")}
            onEdit={() => { setOpenSection("edition"); resetFrom("edition") }}
          >
            <p className="reg-step-sub" style={{ marginBottom:"1.25rem" }}>{t("ef_edition_sub")}</p>
            <div style={{ display:"flex", flexDirection:"column", gap:"0.75rem" }}>
              <SelCard selected={edition === "nl"}            onSelect={() => confirmEdition("nl")}            title={t("ef_edition_nl_title")}   desc={t("ef_edition_nl_desc")} />
              <SelCard selected={edition === "international"} onSelect={() => confirmEdition("international")} title={t("ef_edition_intl_title")} desc={t("ef_edition_intl_desc")} />
            </div>
          </Accordion>

          {/* ── SECTIE 4: Toegangsmethode + telefoon + submit ── */}
          <Accordion
            ...
            summary={null}
            onOpen={() => isUnlocked("access") && setOpenSection("access")}
            onEdit={null}
          >
            <p className="reg-step-sub" style={{ marginBottom:"1.25rem" }}>{t("ef_access_sub")}</p>
            <div style={{ display:"flex", flexDirection:"column", gap:"0.75rem", marginBottom:"1.5rem" }}>
              <AccCard selected={accessMethod === "domain"}  onSelect={() => setAccessMethod("domain")}  icon={<DomainIcon />} title={t("ef_access_domain_title")}  desc={t("ef_access_domain_desc")} />
              <AccCard selected={accessMethod === "sso"}     onSelect={() => setAccessMethod("sso")}     icon={<SsoIcon />}    title={t("ef_access_sso_title")}    desc={t("ef_access_sso_desc")} />
              <AccCard selected={accessMethod === "unknown"} onSelect={() => setAccessMethod("unknown")} icon={<UnknownIcon />} title={t("ef_access_unknown_title")} desc={t("ef_access_unknown_desc")} />
            </div>

            <div className="input-group">
              <label className="input-label">
                {t("ef_phone_label")} <span style={{ color:C.gray500, fontSize:"0.7rem", fontWeight:400 }}>({t("bf_optional")})</span>
              </label>
              <input className="input-field" type="tel" placeholder="+31 6 12 34 56 78" value={phone} onChange={e => setPhone(e.target.value)} />
            </div>

            <label style={{ display:"flex", alignItems:"flex-start", gap:"0.625rem", marginBottom:"1.5rem", cursor:"pointer" }}>
              <input type="checkbox" checked={agreed} onChange={e => setAgreed(e.target.checked)} style={{ marginTop:3, accentColor:C.red }} />
            ...
            <button className="btn-red btn-full" disabled={!accessMethod || !agreed} onClick={() => setSubmitted(true)}>
              {t("ef_submit")}
            </button>
          </Accordion>
...
// Accordion
// ══════════════════════════════════════════════════════════════════════════════
function Accordion({ open, done, unlocked, title, summary, onOpen, onEdit, children }) {
  ...
  return (
    <div style={{ border:`1.5px solid ${open ? C.navy : done ? C.gray200 : C.gray200}`, borderRadius:8, marginBottom:"0.75rem", overflow:"hidden", opacity: locked ? 0.4 : 1, transition:"opacity 0.2s, border-color 0.2s" }}>
      <div onClick={!locked && !open ? onOpen : undefined} style={{ display:"flex", alignItems:"center", justifyContent:"space-between", padding:"0.875rem 1.125rem", background: open ? C.white : done ? C.gray50 : C.white, cursor: !locked && !open ? "pointer" : "default" }}>
        ...
        <div style={{ display:"flex", alignItems:"center", gap:"0.75rem" }}>
          {done && !open && onEdit && (
            <button className="link-btn" style={{ fontSize:"0.8rem" }} onClick={e => { e.stopPropagation(); onEdit() }}>
              {/* "Wijzig" is hardcoded — in productie via i18n */}
              Wijzig
            </button>
          )}
          {!done && !open && !locked && (
